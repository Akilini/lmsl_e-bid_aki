...
function generateTenderRefNo($con, $type)
{
    ...
    function generateTenderRefNo($con, $type)
    {
        ...
        $sql = "SELECT tender_ref_no FROM tender 
        ...
        if ($row = mysqli_fetch_assoc($result)) {
            $parts = explode("/", $row['tender_ref_no']);
        ...
        return "LMS/MKT/TDR/$yearShort/$newNumber";
        }
        ...
        return "LMSL/SUP/DPC/$newNumber/$yearFull";
        }
    }
}
...
if(isset($_POST["btnsave"]))
    {
        ...
        $sqlinsert="INSERT INTO tender (tender_id,tender_ref_no,title,tender_type,currency,publish_date,bid_open_date,bid_close_date,bid_validity,status,create_by)
...
document.querySelectorAll("input[name='txttender_type']").forEach(el => {
    el.addEventListener("change", function(){
        ...
        let yearShort = yearFull.toString().slice(-2);

        if(type === "Sales"){
            document.getElementById("txttender_ref_no").value = "LMS/MKT/TDR/" + yearShort + "/01";
        } else {
            document.getElementById("txttender_ref_no").value = "LMSL/SUP/DPC/01/" + yearFull;
        }
    });
});
</script>