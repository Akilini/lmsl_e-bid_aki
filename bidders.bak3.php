...
if(isset($_POST["btnsave"]))
    ...
    if (isset($_FILES["txtbusiness_registration_copy"]) && $_FILES["txtbusiness_registration_copy"]["error"] == 0) {
        $business_registration_copy = time() . "_" . basename($_FILES["txtbusiness_registration_copy"]["name"]);
        $target_dir1 = "bidders/business_registration_copy/";
        $target_file1 = $target_dir1 . $business_registration_copy;

        if (!is_dir($target_dir1)) {
            mkdir($target_dir1, 0777, true);
        }

        move_uploaded_file($_FILES["txtbusiness_registration_copy"]["tmp_name"], $target_file1);
    }

    // Upload VAT Registration Copy
    if (isset($_FILES["txtvat_registration_copy"]) && $_FILES["txtvat_registration_copy"]["error"] == 0) {
        $vat_registration_copy = time() . "_" . basename($_FILES["txtvat_registration_copy"]["name"]);
        $target_dir2 = "bidders/vat_registration_copy/";
        $target_file2 = $target_dir2 . $vat_registration_copy;

        if (!is_dir($target_dir2)) {
            mkdir($target_dir2, 0777, true);
        }

        move_uploaded_file($_FILES["txtvat_registration_copy"]["tmp_name"], $target_file2);
    }
...
if(isset($_POST["btnupdate"]))
    {
        ...
        // Replace Business Registration Copy if new file uploaded
        if (isset($_FILES["txtbusiness_registration_copy"]) && $_FILES["txtbusiness_registration_copy"]["error"] == 0) {

            $new_business_file = time() . "_" . basename($_FILES["txtbusiness_registration_copy"]["name"]);
            $target_dir1 = "bidders/business_registration_copy/";
            $target_file1 = $target_dir1 . $new_business_file;
            ...
            if (move_uploaded_file($_FILES["txtbusiness_registration_copy"]["tmp_name"], $target_file1)) {
                ...
                if (!empty($business_registration_copy) && file_exists($target_dir1 . $business_registration_copy)) {
                    unlink($target_dir1 . $business_registration_copy);
                }
        ...
        // Replace VAT Registration Copy if new file uploaded
        if (isset($_FILES["txtvat_registration_copy"]) && $_FILES["txtvat_registration_copy"]["error"] == 0) {

            $new_vat_file = time() . "_" . basename($_FILES["txtvat_registration_copy"]["name"]);
            $target_dir2 = "bidders/vat_registration_copy/";
            $target_file2 = $target_dir2 . $new_vat_file;
            ...
            if (move_uploaded_file($_FILES["txtvat_registration_copy"]["tmp_name"], $target_file2)) {
                ...
                // delete old file if exists
                if (!empty($vat_registration_copy) && file_exists($target_dir2 . $vat_registration_copy)) {
                    unlink($target_dir2 . $vat_registration_copy);
                }
